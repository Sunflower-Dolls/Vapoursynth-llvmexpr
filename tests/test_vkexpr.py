...
def test_arithmetic(
    ...
    res = core.llvmexpr.VkExpr([c1, c2], expr, vs.GRAYS)
...
def test_exp(input_format: int, input_value: int, expr: str, expected: float) -> None:
    ...
    result = core.llvmexpr.VkExpr(clip, expr, vs.GRAYS)
...
def test_rounding_and_misc(val: float, expr: str, expected: float) -> None:
    ...
    res = core.llvmexpr.VkExpr(c, expr, vs.GRAYS)
    assert res.get_frame(0)[0][0, 0] == pytest.approx(expected)
...
def test_min_max_clip(val: float, min_v: float, max_v: float) -> None:
    c = core.std.BlankClip(format=vs.GRAYS, color=val)
    res_max = core.llvmexpr.VkExpr(c, "x 3 max", vs.GRAYS)
    res_min = core.llvmexpr.VkExpr(c, "x 3 min", vs.GRAYS)
    res_clip = core.llvmexpr.VkExpr(c, f"x {min_v} {max_v} clip", vs.GRAYS)
...
def test_stack_manipulation() -> None:
    ...
    res_dup = core.llvmexpr.VkExpr(c, "x dup *", vs.GRAYS)
    assert res_dup.get_frame(0)[0][0, 0] == pytest.approx(9.0)
    c0 = core.std.BlankClip(format=vs.GRAYS, color=0.0)
    res_swap = core.llvmexpr.VkExpr(c0, "2 5 swap -", vs.GRAYS)
    assert res_swap.get_frame(0)[0][0, 0] == pytest.approx(3.0)
    res_drop = core.llvmexpr.VkExpr(c0, "1 2 3 drop2", vs.GRAYS)
    assert res_drop.get_frame(0)[0][0, 0] == pytest.approx(1.0)
    res_sort = core.llvmexpr.VkExpr(c0, "3 1 2 sort3 drop2", vs.GRAYS)
...
def test_named_variables_and_loop_power() -> None:
    ...
    res = core.llvmexpr.VkExpr([c, y], expr, vs.GRAYS)
...
def test_constants_and_coords() -> None:
    ...
    res_pi = core.llvmexpr.VkExpr(c0, "pi", vs.GRAYS)
    assert res_pi.get_frame(0)[0][0, 0] == pytest.approx(3.14159265, rel=1e-6)
    res_N = core.llvmexpr.VkExpr(c0, "N", vs.GRAYS)
    assert res_N.get_frame(3)[0][0, 0] == pytest.approx(3.0)
    res_wh = core.llvmexpr.VkExpr(c0, "width height +", vs.GRAYS)
    assert res_wh.get_frame(0)[0][0, 0] == pytest.approx(5.0)
    res_X = core.llvmexpr.VkExpr(c0, "X", vs.GRAYS)
    res_Y = core.llvmexpr.VkExpr(c0, "Y", vs.GRAYS)
...
def test_conditional_ternary() -> None:
    ...
    res = core.llvmexpr.VkExpr(c, "x 5 > 1 0 ?", vs.GRAYS)
...
def test_pixel_access_static_and_dynamic() -> None:
    ...
    ramp = core.llvmexpr.VkExpr(base, "X", vs.GRAYS)
    src = core.std.BlankClip(format=vs.GRAYS, color=99.0, width=4, height=2)
    expr_rel = "y[-1,0]"
    res_rel = core.llvmexpr.VkExpr([src, ramp], expr_rel, vs.GRAYS)
    f = res_rel.get_frame(0)
    assert f[0][0, 2] == pytest.approx(1.0)
    expr_abs = "1 1 y[]"
    res_abs = core.llvmexpr.VkExpr([src, ramp], expr_abs, vs.GRAYS)
...
def test_frame_property_access() -> None:
    ...
    res = core.llvmexpr.VkExpr(c, "x._TestProp", vs.GRAYS)
...
def test_frame_property_exists() -> None:
    ...
    res_exists = core.llvmexpr.VkExpr(c_with_prop, "x._TestProp?", vs.GRAYS)
    assert res_exists.get_frame(0)[0][0, 0] == pytest.approx(1.0)

    res_not_exists = core.llvmexpr.VkExpr(c, "x._MissingProp?", vs.GRAYS)
...
def test_direct_output_write_and_exit() -> None:
    ...
    res = core.llvmexpr.VkExpr(base, expr, vs.GRAYS)
    fr = res.get_frame(0)
...
def test_boundary_conditions(
    ...
    if boundary:
        res = core.llvmexpr.VkExpr(ramp_clip, expr, boundary=boundary)
    else:
        res = core.llvmexpr.VkExpr(ramp_clip, expr)
...
def test_non_integer_coordinate_rounding() -> None:
    ...
    c = core.llvmexpr.VkExpr(c, "X")
    res = core.llvmexpr.VkExpr(c, "X 0.5 + Y 0.5 + x[]", vs.GRAYS)
...
def subsampled_ramp_clip() -> vs.VideoNode:
    ...
    u_ramp_expr = "Y 2 * X +"
    return core.llvmexpr.VkExpr([base], ["", u_ramp_expr])
...
def test_subsampled_plane_access(
    ...
    res = core.llvmexpr.VkExpr(subsampled_ramp_clip, ["", expr])
    frame = res.get_frame(0)
...
def test_array_static_allocation_basic():
    ...
    res = core.llvmexpr.VkExpr(clip, expr, vs.GRAYS)
...
def test_array_dynamic_allocation_error():
    ...
    with pytest.raises(
        ...
        core.llvmexpr.VkExpr(clip, "10 arr{}^ 0", vs.GRAYS)
...
def test_array_uninitialized_error():
    ...
    with pytest.raises(vs.Error, match="Array is uninitialized"):
        core.llvmexpr.VkExpr(clip, "0 arr{}@", vs.GRAYS)
...
def test_clip_dim_tokens_disabled_in_expr(expr: str):
    ...
    with pytest.raises(vs.Error, match="Invalid token"):
        core.llvmexpr.VkExpr([clip1, clip2, clip3, clip4], expr)
